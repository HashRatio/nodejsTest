...
function ifBuySell(records,ticker,account){
	...
	if (record.Volume > averageVolume * 3 && priceChange > averageChange * 2 ) {
		...
		if(record.Close>record.Open){
			buyOrSell = buy(ticker,account);
		}else{
			buyOrSell = sell(ticker,account);
		}
		localLog("AverageVolume:" + averageVolume + ",AverageChange" + averageChange +
					",Volume:" + record.Volume + " ,PriceChange:" + priceChange +
					",BiggestVolume:" + biggestVolume + " ,Date:" + biggestVolumeDate +
					",BiggestChange:" + biggestChange + " ,Date:" + biggestPriceChangeDate );
	}
}
